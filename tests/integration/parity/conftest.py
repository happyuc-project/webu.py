...
def author(parity_fixture_data):
    # need the address to unlock before webu module has been opened
...
@pytest.fixture(scope="module")
def math_contract(webu, math_contract_factory, parity_fixture_data):
    return math_contract_factory(address=parity_fixture_data['math_address'])


@pytest.fixture(scope="module")
def emitter_contract(webu, emitter_contract_factory, parity_fixture_data):
...
@pytest.fixture(scope="module")
def empty_block(webu, parity_fixture_data):
    block = webu.eth.getBlock(parity_fixture_data['empty_block_hash'])
...
@pytest.fixture(scope="module")
def block_with_txn_with_log(webu, parity_fixture_data):
    block = webu.eth.getBlock(parity_fixture_data['block_hash_with_log'])
    assert is_dict(block)
    return block
