...
GETH_16_FIXTURE = {
    'datadir': 'ghuc-16-datadir-fixture',
...
GETH_17_FIXTURE = {
    'datadir': 'ghuc-17-datadir-fixture',
...
GETH_181_DIRECTORY_NAME = 'ghuc-1.8.1-datadir-fixture'
...
@pytest.fixture(scope="module")
def ghuc_fixture_data(ghuc_binary):
    from ghuc import get_ghuc_version
    version = get_ghuc_version(ghuc_executable=os.path.expanduser(ghuc_binary))
    if version.major == 1:
        if version.minor == 6:
    ...
    assert False, "Unsupported ghuc version"
...
@pytest.fixture(scope='module')
def ghuc_process(ghuc_binary, datadir, genesis_file, ghuc_command_arguments):
...
@pytest.fixture(scope='module')
def coinbase(webu):
    return webu.eth.coinbase


@pytest.fixture(scope="module")
def math_contract_deploy_txn_hash(ghuc_fixture_data):
    return ghuc_fixture_data['math_deploy_txn_hash']


@pytest.fixture(scope="module")
def math_contract(webu, math_contract_factory, ghuc_fixture_data):
    return math_contract_factory(address=ghuc_fixture_data['math_address'])


@pytest.fixture(scope="module")
def emitter_contract(webu, emitter_contract_factory, ghuc_fixture_data):
    return emitter_contract_factory(address=ghuc_fixture_data['emitter_address'])
...
@pytest.fixture(scope='module')
def unlockable_account_pw(ghuc_fixture_data):
    return ghuc_fixture_data['keyfile_pw']


@pytest.fixture(scope="module")
def unlockable_account(webu, coinbase):
    yield coinbase


@pytest.fixture(scope="module")
def funded_account_for_raw_txn(ghuc_fixture_data):
    account = ghuc_fixture_data['raw_txn_account']
    assert is_checksum_address(account)
    return account


@pytest.fixture(scope="module")
def empty_block(webu, ghuc_fixture_data):
    block = webu.eth.getBlock(ghuc_fixture_data['empty_block_hash'])
    assert is_dict(block)
    return block


@pytest.fixture(scope="module")
def block_with_txn(webu, ghuc_fixture_data):
    block = webu.eth.getBlock(ghuc_fixture_data['block_with_txn_hash'])
    assert is_dict(block)
    return block


@pytest.fixture(scope="module")
def mined_txn_hash(ghuc_fixture_data):
    return ghuc_fixture_data['mined_txn_hash']


@pytest.fixture(scope="module")
def block_with_txn_with_log(webu, ghuc_fixture_data):
    block = webu.eth.getBlock(ghuc_fixture_data['block_hash_with_log'])
    assert is_dict(block)
    return block


@pytest.fixture(scope="module")
def txn_hash_with_log(ghuc_fixture_data):
    return ghuc_fixture_data['txn_hash_with_log']
