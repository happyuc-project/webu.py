...
A Webu instance configured to connect to the backend for the integration test.


#### `empty_block`

A block as returned by `webu.eth.getBlock` that has no transactions.


#### `block_with_txn`

A block as returned by `webu.eth.getBlock` that has a single transaction.


#### `math_contract`

An deployed Contract instance of the *Math* contract found in
`webu.utils.module_testing.math_contract`.
...
`webu.utils.module_testing.emitter_contract`.
