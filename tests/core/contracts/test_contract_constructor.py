...
def test_contract_constructor_gas_estimate_no_constructor(MathContract):
    ...
    try:
        assert abs(gas_estimate - 167412) < 200  # Ghuc
...
def test_contract_constructor_gas_estimate_with_constructor_without_arguments(
    ...
    try:
        assert abs(gas_estimate - 43082) < 200  # Ghuc
    except AssertionError:
...
def test_contract_constructor_gas_estimate_with_constructor_with_arguments(
    ...
    try:
        assert abs(gas_estimate - 78572) < 200  # Ghuc
    except AssertionError:
...
def test_contract_constructor_gas_estimate_with_constructor_with_address_argument(
    ...
    try:
        assert abs(gas_estimate - 50181) < 200  # Ghuc
    except AssertionError:
        assert abs(gas_estimate - 95399) < 200  # eth-tester with py-evm


def test_contract_constructor_transact_no_constructor(webu, MathContract, MATH_RUNTIME):
...
def test_contract_constructor_transact_with_constructor_without_arguments(
        webu, SimpleConstructorContract, SIMPLE_CONSTRUCTOR_RUNTIME):
...
def test_contract_constructor_transact_with_constructor_with_address_arguments(
        webu, WithConstructorAddressArgumentsContract, WITH_CONSTRUCTOR_ADDRESS_RUNTIME):
...
def test_contract_constructor_build_transaction_no_constructor(webu, MathContract):
    txn_hash = MathContract.constructor().transact({'from': webu.eth.accounts[0]})
    ...
    new_txn_hash = webu.eth.sendTransaction(unsent_txn)
    new_txn = webu.eth.getTransaction(new_txn_hash)
    assert new_txn['data'] == unsent_txn['data']
    assert new_txn['nonce'] == nonce


def test_contract_constructor_build_transaction_with_constructor_without_argument(webu,
                                                                                  MathContract):
    txn_hash = MathContract.constructor().transact({'from': webu.eth.accounts[0]})
    txn = webu.eth.getTransaction(txn_hash)
    nonce = webu.eth.getTransactionCount(webu.eth.coinbase)
...
def test_contract_constructor_build_transaction_with_constructor_with_argument(
    ...
    txn_hash = WithConstructorArgumentsContract.constructor(
        *constructor_args, **constructor_kwargs).transact({'from': webu.eth.accounts[0]})
    txn = webu.eth.getTransaction(txn_hash)
    nonce = webu.eth.getTransactionCount(webu.eth.coinbase)
