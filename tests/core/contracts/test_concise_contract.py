...
@pytest.fixture()
def zero_address_contract(webu, WithConstructorAddressArgumentsContract, EMPTY_ADDR):
...
def test_class_construction_sets_class_vars(webu,
    ...
    assert classic.webu == webu
    assert classic.bytecode == decode_hex(MATH_CODE)
    assert classic.bytecode_runtime == decode_hex(MATH_RUNTIME)


def test_conciscecontract_keeps_custom_normalizers_on_base(webu):
    base_contract = webu.eth.contract()
...
def test_conciscecontract_function_collision(
    ...
    contract = deploy(webu, StringContract, args=["blarg"])
