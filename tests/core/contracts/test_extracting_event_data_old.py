...
@pytest.fixture()
def Emitter(webu, EMITTER):
    return webu.eth.contract(**EMITTER)


@pytest.fixture()
def emitter(webu, Emitter, wait_for_transaction, wait_for_block):
    wait_for_block(webu)
    deploy_txn_hash = Emitter.deploy({'from': webu.eth.coinbase, 'gas': 1000000})
    deploy_receipt = wait_for_transaction(webu, deploy_txn_hash)
    contract_address = deploy_receipt['contractAddress']

    bytecode = webu.eth.getCode(contract_address)
...
def test_event_data_extraction(webu,
                               emitter,
...
def test_dynamic_length_argument_extraction(webu,
                                            emitter,
                                            wait_for_transaction,
                                            emitter_log_topics,
                                            emitter_event_ids):
    ...
    string_0_topic = webu.sha3(text=string_0)
