...
def test_finds_single_function_without_args(webu):
    Contract = webu.eth.contract(abi=SINGLE_FN_NO_ARGS)
...
def test_finds_single_function_with_args(webu):
    Contract = webu.eth.contract(abi=SINGLE_FN_ONE_ARG)
...
def test_finds_fallback_function(webu):
    Contract = webu.eth.contract(abi=FALLBACK_FUNCTION)

    abi = Contract._find_matching_fn_abi(FallbackFn, [])
    assert abi['type'] == 'fallback'


def test_error_when_no_function_name_match(webu):
    Contract = webu.eth.contract(abi=SINGLE_FN_NO_ARGS)
...
def test_finds_function_with_matching_args(webu, arguments, expected_types):
    Contract = webu.eth.contract(abi=MULTIPLE_FUNCTIONS)
...
def test_error_when_duplicate_match(webu):
    Contract = webu.eth.contract(abi=MULTIPLE_FUNCTIONS)
