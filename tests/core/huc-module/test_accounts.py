...
# from https://github.com/happyuc-project/tests/blob/3930ca3a9a377107d5792b3e7202f79c688f1a67/BasicTests/txtest.json # noqa: 501
...
def acct(request, webu):
    if request.param == 'instance':
        return webu.eth.account
...
@pytest.mark.parametrize(
    ...
    ids=['webujs_example', '31byte_r_and_s'],
)
...
def test_eth_account_encrypt(acct, webujs_key, webujs_password):
    encrypted = acct.encrypt(webujs_key, webujs_password)

    assert encrypted['address'] == '2c7536e3605d9c16a7a3d7b1898e529396a65c23'
    assert encrypted['version'] == 3

    decrypted_key = acct.decrypt(encrypted, webujs_password)

    assert decrypted_key == to_bytes(hexstr=webujs_key)


def test_eth_account_prepared_encrypt(acct, webujs_key, webujs_password):
    account = acct.privateKeyToAccount(webujs_key)
    encrypted = account.encrypt(webujs_password)
