...
import webu
from webu.middleware.exception_retry_request import (
...
def exception_retry_request_setup():
    ...
    setup = exception_retry_middleware(provider.make_request, webu, errors, 5)
    setup.webu = webu
    return setup
...
@patch('webu.providers.rpc.make_post_request', side_effect=ConnectionError)
...
def test_is_strictly_default_http_middleware():
    webu = HTTPProvider()
    assert 'http_retry_request' in webu.middlewares

    webu = IPCProvider()
    assert 'http_retry_request' not in webu.middlewares


@patch('webu.providers.rpc.make_post_request', side_effect=ConnectionError)
def test_check_with_all_middlewares(make_post_request_mock):
    provider = HTTPProvider()
    w3 = webu.Webu(provider)
    with pytest.raises(ConnectionError):
        w3.eth.blockNumber()
    assert make_post_request_mock.call_count == 5
