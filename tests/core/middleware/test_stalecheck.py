...
from webu.middleware.stalecheck import (
    StaleBlockchain,
    _isfresh,
)
...
def request_middleware(allowable_delay):
    ...
    initialized = middleware(make_request, webu)
...
def test_stalecheck_pass(request_middleware):
    with patch('webu.middleware.stalecheck._isfresh', return_value=True):
...
def test_stalecheck_fail(request_middleware, now):
    with patch('webu.middleware.stalecheck._isfresh', return_value=False):
        request_middleware.webu.eth.getBlock.return_value = stub_block(now)
...
def test_stalecheck_ignores_get_by_block_methods(request_middleware, rpc_method):
    ...
    with patch('webu.middleware.stalecheck._isfresh', side_effect=[False, True]):
        request_middleware(rpc_method, [])
        assert not request_middleware.webu.eth.getBlock.called


def test_stalecheck_calls_isfresh_with_empty_cache(request_middleware, allowable_delay):
    with patch('webu.middleware.stalecheck._isfresh', side_effect=[False, True]) as freshspy:
        block = object()
        request_middleware.webu.eth.getBlock.return_value = block
        request_middleware('', [])
...
def test_stalecheck_adds_block_to_cache(request_middleware, allowable_delay):
    with patch('webu.middleware.stalecheck._isfresh', side_effect=[False, True, True]) as freshspy:
        block = object()
        request_middleware.webu.eth.getBlock.return_value = block
