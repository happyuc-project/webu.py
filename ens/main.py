...
class ENS:
    ...
    `checksum format <https://github.com/happyuc-project/EIPs/blob/master/EIPS/eip-155.md>`_,
    ...
    def __init__(self, providers=default, addr=None):
        ...
        '''
        self.webu = init_webu(providers)

        ens_addr = addr if addr else ENS_MAINNET_ADDR
        self.ens = self.webu.eth.contract(abi=abis.ENS, address=ens_addr)
        self._resolverContract = self.webu.eth.contract(abi=abis.RESOLVER)

    @classmethod
    def fromWebu(cls, webu, addr=None):
        '''
        Generate an ENS instance with webu

        :param `webu.Webu` webu: to infer connection information
        :param hex-string addr: the address of the ENS registry on-chain. If not provided,
            ENS.py will default to the mainnet ENS registry address.
        '''
        return cls(webu.manager.providers, addr=addr)
    ...
    def _assert_control(self, account, name, parent_owned=None):
        if not address_in(account, self.webu.eth.accounts):
    ...
    def _reverse_registrar(self):
        ...
        return self.webu.eth.contract(address=addr, abi=abis.REVERSE_REGISTRAR)
