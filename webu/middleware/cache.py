...
from webu.utils.caching import (
    generate_cache_key,
)
...
def construct_simple_cache_middleware(
    ...
    :param should_cache_fn: A callable which accepts ``method`` ``params`` and
        ...
        cached.
    """
    def simple_cache_middleware(make_request, webu):
        cache = cache_class()
        lock = threading.Lock()
...
def construct_time_based_cache_middleware(
    ...
    :param should_cache_fn: A callable which accepts ``method`` ``params`` and
        ...
        cached.
    """
    def time_based_cache_middleware(make_request, webu):
        cache = cache_class()
        lock = threading.Lock()
...
def construct_latest_block_based_cache_middleware(
    ...
    .. note::
        ...
        block time.
    """
    def latest_block_based_cache_middleware(make_request, webu):
        cache = cache_class()
        block_info = {}
        ...
        def _update_block_info_cache():
            ...
            if avg_block_time_age_in_blocks >= avg_block_sample_size:
                ...
                # time.
                latest_block = webu.eth.getBlock('latest')
                ancestor_block_number = max(
                    0,
                    latest_block['number'] - average_block_time_sample_size,
                )
                ancestor_block = webu.eth.getBlock(ancestor_block_number)
            ...
            if 'latest_block' in block_info:
                ...
                if time_since_latest_block > avg_block_time:
                    block_info['latest_block'] = webu.eth.getBlock('latest')
            else:
