...
from webu.utils.formatters import (
    apply_formatter_if,
...
transaction_params_formatter = compose(
    # remove nonce for now due to issue https://github.com/happyuc-project/eth-tester/issues/80
...
def guess_from(webu, transaction):
    coinbase = webu.eth.coinbase
    if coinbase is not None:
        return coinbase

    try:
        return webu.eth.accounts[0]
...
def guess_gas(webu, transaction):
    return webu.eth.estimateGas(transaction) * 2


@curry
def fill_default(field, guess_func, webu, transaction):
    if field in transaction and transaction[field] is not None:
        return transaction
    else:
        guess_val = guess_func(webu, transaction)
        return assoc(transaction, field, guess_val)


def default_transaction_fields_middleware(make_request, webu):
    fill_default_from = fill_default('from', guess_from, webu)
    fill_default_gas = fill_default('gas', guess_gas, webu)
