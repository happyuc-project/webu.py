...
class BaseProvider:
    ...
    def request_func(self, webu, outer_middlewares):
        '''
        ...
        if cache_key is None or cache_key != all_middlewares:
            self._request_func_cache = (
                all_middlewares,
                self._generate_request_func(webu, all_middlewares)
            )
        return self._request_func_cache[-1]

    def _generate_request_func(self, webu, middlewares):
...
class JSONBaseProvider(BaseProvider):
    ...
    def isConnected(self):
        try:
            response = self.make_request('webu_clientVersion', [])
        except IOError:
            return False
        else:
