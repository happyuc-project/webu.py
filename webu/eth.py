...
class Eth(Module):
    ...
    @property
    def syncing(self):
        return self.webu.manager.request_blocking("eth_syncing", [])

    @property
    def coinbase(self):
        return self.webu.manager.request_blocking("eth_coinbase", [])

    @property
    def mining(self):
        return self.webu.manager.request_blocking("eth_mining", [])
    ...
    @property
    def gasPrice(self):
        return self.webu.manager.request_blocking("eth_gasPrice", [])

    @property
    def accounts(self):
        return self.webu.manager.request_blocking("eth_accounts", [])

    @property
    def blockNumber(self):
        return self.webu.manager.request_blocking("eth_blockNumber", [])
    ...
    def waitForTransactionReceipt(self, transaction_hash, timeout=120):
        return wait_for_transaction_receipt(self.webu, transaction_hash, timeout)
    ...
    def replaceTransaction(self, transaction_hash, new_transaction):
        current_transaction = get_required_transaction(self.webu, transaction_hash)
        return replace_transaction(self.webu, current_transaction, new_transaction)
    ...
    def sendTransaction(self, transaction):
        ...
        if 'gas' not in transaction:
            transaction = assoc(
                transaction,
                'gas',
                get_buffered_gas_estimate(self.webu, transaction),
            )
    ...
    def filter(self, filter_params=None, filter_id=None):
        ...
        if is_string(filter_params):
            if filter_params == "latest":
                filter_id = self.webu.manager.request_blocking(
                    "eth_newBlockFilter", [],
                )
                return BlockFilter(self.webu, filter_id)
            elif filter_params == "pending":
                filter_id = self.webu.manager.request_blocking(
                    "eth_newPendingTransactionFilter", [],
                )
                return TransactionFilter(self.webu, filter_id)
            else:
                raise ValueError(
            ...
            _filter_id = self.webu.manager.request_blocking(
                "eth_newFilter",
                [filter_params],
            )
            return LogFilter(self.webu, _filter_id)
        elif filter_id and not filter_params:
            return LogFilter(self.webu, filter_id)
        else:
    ...
    def contract(self,
        ...
        ContractFactory = ContractFactoryClass.factory(self.webu, **kwargs)
    ...
    def getCompilers(self):
        return self.webu.manager.request_blocking("eth_getCompilers", [])

    def getWork(self):
        return self.webu.manager.request_blocking("eth_getWork", [])

    def generateGasPrice(self, transaction_params=None):
        if self.gasPriceStrategy:
            return self.gasPriceStrategy(self.webu, transaction_params)
