"""Interaction with smart contracts over Webu connector.
...
from webu.utils.blocks import (
    is_hex_encoded_block_hash,
)
from webu.utils.contracts import (
    encode_abi,
...
from webu.utils.datatypes import (
...
from webu.utils.filters import (
    construct_event_filter_params,
)
from webu.utils.function_identifiers import (
    FallbackFn,
)
...
class ContractFunctions:
    ...
    def __init__(self, abi, webu, address=None):
        if abi:
            self.abi = abi
...
class Contract:
    ...
    :meth:`webu.eth.Eth.contract` that takes compiled Solidity contract
    ...
    * Deploy a new smart contract using :py:meth:`Contract.deploy`
    ...
    def __init__(self, address=None):
        ...
        """
        if self.webu is None:
            raise AttributeError(
                'The `Contract` class has not been initialized.  Please use the '
                '`webu.contract` interface to create your contract class.'
            )

        if address:
            self.address = normalize_address(self.webu.ens, address)

        if not self.address:
            raise TypeError("The address argument is required to instantiate a contract.")

        self.functions = ContractFunctions(self.abi, self.webu, self.address)
        self.events = ContractEvents(self.abi, self.webu, self.address)
        self.fallback = Contract.get_fallback_function(self.abi, self.webu, self.address)

    @classmethod
    def factory(cls, webu, class_name=None, **kwargs):

        kwargs['webu'] = webu

        normalizers = {
            'abi': normalize_abi,
            'address': partial(normalize_address, kwargs['webu'].ens),
        ...
        setattr(contract, 'functions', ContractFunctions(contract.abi, contract.webu))
        setattr(contract, 'events', ContractEvents(contract.abi, contract.webu))
        setattr(contract, 'fallback', Contract.get_fallback_function(contract.abi, contract.webu))
    ...
    def deploy(cls, transaction=None, args=None, kwargs=None):
        ...
        txn_hash = cls.webu.eth.sendTransaction(deploy_transaction)
        return txn_hash
    ...
    def constructor(cls, *args, **kwargs):
        ...
        return ContractConstructor(cls.webu,
                                   cls.abi,
                                   cls.bytecode,
                                   *args,
                                   **kwargs)
    ...
    def encodeABI(cls, fn_name, args=None, kwargs=None, data=None):
        ...
        return encode_abi(cls.webu, fn_abi, fn_arguments, data)
    ...
    def eventFilter(self, event_name, filter_params={}):
        ...
        log_filter = self.webu.eth.filter(event_filter_params)
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.webu.eth.defaultAccount is not empty:
            estimate_transaction.setdefault('from', self.webu.eth.defaultAccount)
    ...
    def call(self, transaction=None):
        ...
        :param transaction: Dictionary of transaction info for webu interface
        ...
        if self.webu.eth.defaultAccount is not empty:
            call_transaction.setdefault('from', self.webu.eth.defaultAccount)
    ...
    def transact(self, transaction=None):
        ...
        .. code-block:: python
            ...
            # Deposit to the `webu.eth.coinbase` account.
            >>> wallet.functions.deposit().transact({'value': 12345})
            '0x5c504ed432cb51138bcf09aa5e8a410dd4a1e204ef84bfed1be16dfba1b22060'
            # Deposit to some other account using funds from `webu.eth.coinbase`.
            >>> wallet.functions.deposit(webu.eth.accounts[1]).transact({'value': 54321})
        ...
        if self.webu.eth.defaultAccount is not empty:
            transact_transaction.setdefault('from', self.webu.eth.defaultAccount)
    ...
    @staticmethod
    def get_fallback_function(abi, webu, address=None):
    ...
    def _encode_constructor_data(cls, args=None, kwargs=None):
        ...
        if constructor_abi:
            ...
            deploy_data = add_0x_prefix(
                encode_abi(cls.webu, constructor_abi, arguments, data=cls.bytecode)
            )
        else:
...
class ContractConstructor:
    ...
    """
    def __init__(self, webu, abi, bytecode, *args, **kwargs):
        self.webu = webu
        self.abi = abi
    ...
    def _encode_data_in_transaction(self, *args, **kwargs):
        ...
        if constructor_abi:
            ...
            arguments = merge_args_and_kwargs(constructor_abi, args, kwargs)
            data = add_0x_prefix(
                encode_abi(self.webu, constructor_abi, arguments, data=self.bytecode)
            )
        else:
    ...
    def estimateGas(self, transaction=None):
        ...
        if self.webu.eth.defaultAccount is not empty:
            estimate_gas_transaction.setdefault('from', self.webu.eth.defaultAccount)

        estimate_gas_transaction['data'] = self.data_in_transaction

        return self.webu.eth.estimateGas(estimate_gas_transaction)
    ...
    def transact(self, transaction=None):
        ...
        return self.webu.eth.sendTransaction(transact_transaction)
    ...
    def buildTransaction(self, transaction=None):
        ...
        if self.webu.eth.defaultAccount is not empty:
            built_transaction.setdefault('from', self.webu.eth.defaultAccount)

        built_transaction['data'] = self.data_in_transaction
        built_transaction['to'] = b''
        return fill_transaction_defaults(self.webu, built_transaction)
...
class ContractFunction:
    ...
    def call(self, transaction=None, block_identifier='latest'):
        ...
        block_id = parse_block_identifier(self.webu, block_identifier)
    ...
    def _encode_transaction_data(cls):
        return add_0x_prefix(encode_abi(cls.webu, cls.abi, cls.arguments, cls.selector))
...
def call_contract_function(abi,
    ...
    if block_id is None:
        return_data = webu.eth.call(call_transaction)
    else:
        return_data = webu.eth.call(call_transaction, block_identifier=block_id)
    ...
    try:
        ...
        is_missing_code_error = (
            ...
            webu.eth.getCode(address) in ACCEPTABLE_EMPTY_STRINGS
        )
...
def parse_block_identifier(webu, block_identifier):
    last_block = webu.eth.getBlock('latest').number
    if isinstance(block_identifier, int) and abs(block_identifier) <= last_block:
        ...
        return webu.eth.getBlock(block_identifier).number
    else:
...
def transact_with_contract_function(abi,
    ...
    txn_hash = webu.eth.sendTransaction(transact_transaction)
    return txn_hash
...
def estimate_gas_for_function(abi,
    ...
    gas_estimate = webu.eth.estimateGas(estimate_transaction)
    return gas_estimate
...
def build_transaction_for_function(abi,
    ...
    prepared_transaction = fill_transaction_defaults(webu, prepared_transaction)
