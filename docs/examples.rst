...
Here are some common things you might want to do with webu.

Looking up blocks
-----------------

Blocks can be looked up by either their number or hash using the
``webu.eth.getBlock`` API.  Block hashes should be in their hexidecimal
representation.  Block numbers

.. code-block:: python

    # get a block by number
    >>> webu.eth.getBlock(12345)
    {
    ...
    >>> webu.eth.getBlock('0x767c2bfb3bdee3f78676c1285cd757bcd5d8c272cef2eb30d9733800a78c0b6d')
    {...}
...
Webu can help you convert between denominations.  The following denominations are supported.
...
You can look up transactions using the ``webu.eth.getTransaction`` function.
...
Transaction receipts can be retrieved using the ``webu.eth.getTransactionReceipt`` API.
...
.. code-block:: python
    ...
    from webu.providers.eth_tester import HappyUCTesterProvider
