...
.. py:module:: webu.utils.filters


The :meth:`webu.eth.Eth.filter` method can be used to setup filters for:

* Pending Transactions: ``webu.eth.filter('pending')``

* New Blocks ``webu.eth.filter('latest')``
...
* Event Logs
    ...
    Or built manually by supplying `valid filter params <https://github.com/happyuc-project/wiki/wiki/JSON-RPC#eth_newfilter/>`_:

    .. code-block:: python

        event_filter = webu.eth.filter({"address": contract_address})
...
* Attaching to an existing filter
    ...
    .. code-block:: python
        ...
        existing_filter = webu.eth.filter(filter_id="0x0")


Filter Class
------------

.. py:class:: Filter(webu, filter_id)
...
.. py:method:: Filter.get_new_entries()
    ...
    :func:`webu.eth.Eth.getFilterChanges` which returns only new entries since the last
    poll.
...
.. py:method:: Filter.get_all_entries()
    ...
    Logs will be retrieved using the
    :func:`webu.eth.Eth.getFilterLogs` which returns all entries that match the given
    filter.
...
You can setup a filter for new blocks using ``webu.eth.filter('latest')`` which
...
You can setup a filter for new blocks using ``webu.eth.filter('pending')`` which
will return a new :py:class:`BlockFilter` object.
...
You can set up a filter for event logs using the webu.py contract api: 
:func:`webu.contract.Contract.events.<event_name>.createFilter`, which provides some conveniances for
...
See :meth:`webu.contract.Contract.events.<event_name>.createFilter` documentation for more information.

You can set up an event log filter like the one above with `webu.eth.filter` by supplying a
...
equivalent filter creation would look like:
    ...
    .. code-block:: python
        ...
        event_signature_hash = webu.sha3(text="eventName(uint32)").hex()
        event_filter = webu.eth.filter({
...
See the JSON-RPC documentation for `eth_newFilter <https://github.com/happyuc-project/wiki/wiki/JSON-RPC#eth_newfilter>`_ more information on the standard filter parameters.

Creating a log filter by either of the above methods will return a :class:`LogFilter` instance.

.. py:class:: LogFilter(webu, filter_id, log_entry_formatter=None, data_filter_set=None)
...
Starting with webu version 4, the ``watch`` method was taken out of the webu filter objects.
There are many decisions to be made when designing a system regarding threading and concurrency.
Rather than force a decision, webu leaves these choices up to the user. Below are some example 
...
an example event loop using asyncio_, that polls multiple webu filter object, and passes new 
