...
By default, access to this module has been turned off in the stable version of Webu.py:
...
Local Node
  ...
  When you run ``ghuc`` or ``parity`` on your machine, you are running a local node.
...
Local Private Key
  ...
  You must use :meth:`~webu.eth.Eth.sendRawTransaction`
...
Hosted Private Key
  ...
  Each account returned by :attr:`w3.eth.accounts <webu.eth.Eth.accounts>`
  has a hosted private key stored in your node.
  This allows you to use :meth:`~webu.eth.Eth.sendTransaction`.
...
Extract private key from ghuc keyfile
---------------------------------------------
...
.. WARNING:: There is no single message format that is broadly adopted
    ...
    like `EIP-683 <https://github.com/happyuc-project/EIPs/pull/683>`_,
    `EIP-712 <https://github.com/happyuc-project/EIPs/pull/712>`_, and
    `EIP-719 <https://github.com/happyuc-project/EIPs/pull/719>`_. Consider
...
validate the value in Remix or webu.js.
...
.. doctest::
    ...
    # Remix / webu.js expect r and s to be encoded to hex
    # This convenience method will do the pad & hex for us:
    >>> def to_32byte_hex(val):
    ...   return Webu.toHex(Webu.toBytes(val).rjust(32, b'\0'))

    >>> ec_recover_args = (msghash, v, r, s) = (
    ...   Webu.toHex(signed_message.messageHash),
    ...   signed_message.v,
...
.. doctest::
    ...
    # Remix / webu.js expect the message hash to be encoded to a hex string
    >>> hex_message_hash = Webu.toHex(message_hash)

    # ecrecover in Solidity expects the signature to be split into v as a uint8,
    #   and r, s as a bytes32
    # Remix / webu.js expect r and s to be encoded to hex
    >>> sig = Webu.toBytes(hexstr=hex_signature)
    >>> v, hex_r, hex_s = Webu.toInt(sig[-1]), Webu.toHex(sig[:32]), Webu.toHex(sig[32:64])
...
with :meth:`~webu.eth.Eth.sendRawTransaction`.
...
#. Initialize your :meth:`Contract <webu.eth.Eth.contract>` object
#. Build the transaction
#. Sign the transaction, with :meth:`w3.eth.account.signTransaction()
   <eth_account.account.Account.signTransaction>`
#. Broadcast the transaction with :meth:`~webu.eth.Eth.sendRawTransaction`
