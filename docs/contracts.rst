...
Instead, create contract objects using the :meth:`w3.eth.contract() <webu.eth.Eth.contract>`
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).transact(transaction=None)
    ...
    ``webu.eth.sendTransaction(transaction)`` method.  This value may not
    ...
    deployment transaction will be created using the ``webu.eth.estimateGas()``
    method.
    ...
    .. code-block:: python
        ...
        >>> deploy_txn = token_contract.constructor(webu.eth.coinbase, 12345).transact()
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).estimateGas(transaction=None)
    ...
    .. code-block:: python
        ...
        >>> token_contract.constructor(webu.eth.coinbase, 12345).estimateGas()
        12563
...
.. py:classmethod:: Contract.constructor(*args, **kwargs).buildTransaction(transaction=None)
    ...
    .. code-block:: python
        ...
        >>> contract_data = token_contract.constructor(webu.eth.coinbase, 12345).buildTransaction(transaction)
        >>> webu.eth.sendTransaction(contract_data)
...
.. py:classmethod:: Contract.events.<event name>.createFilter(fromBlock=block, [toBlock=block, argument_filters={"arg1": "value"}, topics=[]])
    ...
    Creates a new event filter, an instance of :py:class:`webu.utils.filters.LogFilter`.
    ...
    ``topics`` optional, accepts the standard JSON-RPC topics argument.  See the JSON-RPC documentation for `eth_newFilter <https://github.com/happyuc-project/wiki/wiki/JSON-RPC#eth_newfilter>`_ more information on the ``topics`` parameters.
...
.. py:classmethod:: Contract.eventFilter(event_name, filter_params=None)
    ...
    Creates a new :py:class:`webu.utils.filters.LogFilter` instance.
...
For example:
    ...
    .. code-block:: python
        ...
        myContract = webu.eth.contract(address=contract_address, abi=contract_abi)
...
.. py:method:: ContractFunction.transact(transaction)
    ...
    the same format as the ``webu.eth.sendTransaction(transaction)`` method.
    ...
    If a ``gas`` value is not provided, then the ``gas`` value for the
    method transaction will be created using the ``webu.eth.estimateGas()``
    method.
    ...
    .. code-block:: python
        ...
        >>> token_contract.functions.transfer(webu.eth.accounts[1], 12345).transact()
...
.. py:method:: ContractFunction.call(transaction)
    ...
    .. code-block:: python
        ...
        21
        >>> token_contract.functions.myBalance().call({'from': webu.eth.coinbase})
        12345  # the token balance for `webu.eth.coinbase`
        >>> token_contract.functions.myBalance().call({'from': webu.eth.accounts[1]})
        54321  # the token balance for the account `webu.eth.accounts[1]`
...
For example:
    ...
    .. code-block:: python
        ...
        receipt = webu.eth.getTransactionReceipt(tx_hash)
